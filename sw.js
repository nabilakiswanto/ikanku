...
self.addEventListener('install', function(event) {
  ...
  event.waitUntil(
    caches.open(ikanku).then(function(cache) {
...
self.addEventListener('activate', function(event) {
  ...
  event.waitUntil(
    caches.keys().then(function(ikanku) {
      return Promise.all(
        ikanku.map(function(ikanku) {
          if (cacheWhitelist.indexOf(ikanku) === -1) {
            return caches.delete(ikanku);
          }
        })
      );
    })
